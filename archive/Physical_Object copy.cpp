...
{
	if(!isBoundingBox()) throw std::invalid_argument("Les points ne forment pas une bounding box");
}
...
bool BoundingBox::isBoundingBox() const{
	...
	if( (top_right[0] - top_left[0]) * (bottom_right[0] - bottom_left[0]) +
		(top_right[1] - top_left[1]) * (bottom_right[1] - bottom_left[1]) != 0)
		return false;
...
Point2DF BoundingBox::getCenter() const{
	...
	return {(top_left[0] + top_right[0] + bottom_left[0] + bottom_right[0]) / 4,
...
Polygon& Convex_Polygon::getPoints() const{
	return points;
}
...
void Convex_Polygon::computeCentroid(){
	...
	centroid = static_cast<Point2DF>{x , y};
...
bool Convex_Polygon::isColliding(const Physical_Object& other, Vector2DF& intersection, Vector2DF& normal) const{
	...
	returnPhysical_Object::isColliding(other, intersection, normal);
}
...
std::vector<Vector2DF> Convex_Polygon::getEdges() const {
	...
	for (auto it1 = points.begin(); it1 != std::prev(points.end()); ++it1) {
		...
		Vector2DF edge = {it2->first - it1->first, it2->second - it1->second};
...
bool Convex_Polygon::isColliding(const Convex_Polygon &other, Vector2DF &intersection_point, Vector2DF &normal) const {
	...
	for (const auto& edge : getEdges(poly1)) {
...
bool Convex_Polygon::isColliding(const Circle &other, Vector2DF &intersection_point, Vector2DF &normal) const {
	...
	// Déterminer les axes de séparation
	std::vector<Vector2DF> axes;
	for (auto it1 = poly.begin(); it1 != std::prev(poly.end()); ++it1) {
		auto it2 = std::next(it1);
		Vector2DF edge = {it2->first - it1->first, it2->second - it1->second};
		axes.push_back({-edge[1], edge[0]}); // Normale à l'arête
	}
	...
	// Trouver le point d'intersection le plus proche
	float min_dist_sq = std::numeric_limits<float>::max();
	for (const auto& edge : getEdges(poly)) {
...
void Circle::computeMinimumBoundingBox(){
	...
	hitbox = BoundingBox({centroid[0] - radius, centroid[1] + radius},
...
bool Circle::isColliding(const Convex_Polygon& other, Vector2DF& intersection_point, Vector2DF& normal) const {
	// Par défaut, supposons qu'il n'y a pas de collision
	bool collision = false;

	// Calculer le vecteur entre les centres des deux objets
	Vector2DF center_distance = other.getPosition() - getPosition();

	// Vérifier si la distance entre les centres est inférieure ou égale à la somme des rayons
	if (center_distance.distance_square({0, 0}) <= (getRadius() + other.getRadius()) * (getRadius() + other.getRadius())) {
		// S'il y a une collision potentielle, effectuer des vérifications plus détaillées
		// Projection de chaque axe du polygone sur la ligne qui relie les centres des deux objets
		std::vector<Vector2DF> axes = other.getAxes();
		axes.push_back(center_distance);

		// Pour chaque axe, projeter les deux objets et vérifier s'il y a un chevauchement
		for (const Vector2DF& axis : axes) {
			// Projeter le cercle sur l'axe
			auto circle_projection = project(axis);

			// Projeter le polygone sur l'axe
			auto polygon_projection = other.project(axis);

			// Vérifier s'il y a chevauchement
			if (!overlap(circle_projection, polygon_projection)) {
				// S'il n'y a pas de chevauchement sur au moins un axe, il n'y a pas de collision
				return false;
			}
		}

		// S'il y a un chevauchement sur tous les axes, il y a collision
		collision = true;
	}
